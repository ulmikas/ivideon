...
const Camera = ({ camera_name, description, total_views, preview, camera, server, height, width, favorite, toggleView, onFavCLick }) => ((!preview)
  ?
    <li className={(preview) ? styles.cameraSelected : styles.camera}>
      {/*<div onClick={toggleView} className={styles.image} style={{ backgroundImage: 'url(http://lorempixel.com/1400/600/sports/1/)' }} />*/}
      <div onClick={toggleView} className={styles.image}>
        <img alt="" style={{ width: Math.max(100, 75 * width / height) + '%' }} src={`https://streaming.ivideon.com/preview/live?server=${server}&camera=${camera}`} />
        {/*<button onClick={toggleView} className={styles.close}>&times;</button>*/}
      </div>
      <div className={styles.info}>
        <div className={styles.name}>{ camera_name }</div>
        {(preview) ? <div className={styles.description}>{ description }</div> : null}
        ...
        <div className={styles.total_views}>{ total_views }</div>
      </div>
    </li>
  :
    <li className={styles.cameraView}>
      ...
      <div className={styles.info}>
        ...
        <div className={styles.total_views}>TOTAL VIEWS: { total_views }</div>
      </div>
    </li>
);
